...
APIDocStruct APIDoc[] = {
    {
        ...
        "Get version numbers of the API and the underlying NeoUnZip code.\n\n"
        "\t\tThis is used for comparing the version numbers of the run-time\n"
        "\t\tDLL code with those expected from the neounzip.h at compile time.\n"
        ...
        "\t\tSee neounzip.h for details and unzipstb.c for an example.\n"
    },
...
\n\
C functions: -- See neounzip.h for details\n\
  UzpVer *UzpVersion(void);\n\
  int UzpMain(int argc, char *argv[]);\n\
  int UzpAltMain(int argc, char *argv[], UzpInit *init);\n\
  int UzpUnzipToMemory(char *zip, char *file, UzpBuffer *retstr);\n\
