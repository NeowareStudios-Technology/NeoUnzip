...
# Generic build including bzip2 decompression support for neounzip.
...
# inflate rather than the original NeoUnzip version.  (libz was libgz prior
# to 0.94)  Need to figure out how to force neounzipsfx to use static libz.
...
# Generic GNU C shared library.  This is an example of how to compile NeoUnzip as
...
generic_shlib:	unix_make
 ...
 'or similar in order to test the shared library in place (with ./unzip_shlib ,'
	@echo\
 'which is NeoUnzip linked with the DLL).  This target is an example only.'
	@echo ""
...
#----------------------------------------------------------------------------
#  "Normal" group (BSD vs. SysV may be set in neounzip.h via predefined macros):
#----------------------------------------------------------------------------
...
bsd:		_bsd	# generic BSD (BSD 4.2 & Ultrix handled in neounzip.h)
...
sysv:		_sysv	# generic System V Unix (Xenix handled in neounzip.h)
...
# now, and it only makes the NeoUnzip/ZipInfo stuff (not fNeoUnzip or NeoUnzipSFX).
# The version number may eventually change to match the NeoUnzip version.  Or
# not.  Whatever.  Also do "setenv LD_LIBRARY_PATH `pwd`" or similar to test
# the DLL in place (with unzip_shlib, which is NeoUnzip linked with the shared
# library).
#
...
# instead of the original NeoUnzip version.  (libz was libgz prior to 0.94)
...
sco_dos:	unix_make
	$(MAKE) unzips CFLAGS="-O -dos -M0" M=msdos OSDEP_H="msdos/doscfg.h" \
	 LF="-dos -F 2000" LF2="-o neounzip.exe" \
	 FL="-dos" FL2="-o fneounzip.exe" SL="-dos" SL2="-o neounzipsfx.exe"
