/*
  ...
  (the contents of which are also included in neounzip.h) for terms of use.
...
/*---------------------------------------------------------------------------
  ...
  In a prior life it was part of the main neounzip.h header, but now it is only
 ...
 * that add the B_flag to the UzpOpts structure, see neounzip.h.)
 */
...
#include <signal.h>      /* used in neounzip.c, fileio.c */
...
#endif
#ifndef USAGE
#  define USAGE(ret)    usage(__G__ (ret))    /* used in neounzip.c, zipinfo.c */
#endif
...
/* (the PK-class error codes are public and have been moved into neounzip.h) */
...
/*---------------------------------------------------------------------------
    Functions in neounzip.c (initialization routines):
  ---------------------------------------------------------------------------*/
...
#ifdef FUNZIP
   int   flush                OF((__GPRO__ ulg size));  /* actually fneounzip.c */
#else
 ...
 * neounzip. Otherwise, to get the same behaviour as for (*G.message)(), the
 ...
 *  and neounzip.c in several functions.
 *
 *  macro SKIP_(length)
 *      ush length;
 *  {
